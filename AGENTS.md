...
**Partial remeshing (incremental mesh rebuilds for block edits):**
- Replaced the full 32³ rebuild on every block edit with an incremental path: dirty-block AABB (`partial_bounds_`) → targeted emit + merge with the previously emitted mesh (vertex/index reuse, growable buffers), keeping `last_partial_bounds_` so a second edit can splice onto the first. Fallback to a full rebuild when bounds grow beyond a threshold (toggle always no-light → light now toggles the light path; the benchmark light grid is static so toggles take the no-light path).
- Fixed a zero-box light-bounds bug (`light_bounds` union ran even when no light column changed, inflating the re-emit region to a full-height slab of the entire chunk) — union now only when `light_changed`, with a regression test asserting tight bounds for a mid-chunk no-light edit (commit `0bf0132`).
- **Tight-region solid_cache population** (see Key Decisions): incremental builds populate `solid_cache` only for a tight box around the re-emit region, with all out-of-box reads routed live through `ChunkNeighborAccessor`. Population cost 0.168 ms → ~0; incremental rebuild 0.255 → 0.094 ms (light-checksum scan now dominant, see Next Steps). Added `debug_poison_solid_cache()` + a greedy canary test that builds a floating-pillar case forcing out-of-region reads — verified it fails (meshes differ) when the live-read fallback is disabled.
...
- **Partial-remesh solid_cache population**: partial rebuilds populate `solid_cache` only for a tight box (`solid_bounds_` = re-emit region + 1 ring, `expand_bounds(partial_bounds_)`) instead of the full 32³ chunk. Every `solid_cache` read in the greedy/fallback emitters goes through `solid_at(y, zi, xi)`, which live-reads the chunk/neighbor data for any cell outside the box. This is what makes the tight box sound: the merge passes scan the FULL merge axes (horizontal pass reads the whole z-row per selected x/y; vertical pass reads the full column height per selected x/z) to find exact run boundaries for runs that straddle the region, so out-of-box cells must be read live, not from a stale cache. AO/light reads were never `solid_cache` consumers — they already went through the live accessor. Population dropped from 0.168 ms to ~0 for single edits; incremental mesh 0.255 → 0.094 ms (light-checksum scan is now the dominant remaining cost). `debug_poison_solid_cache()` (test-only) fills the cache with an invalid sentinel so any unpopulated read fails loudly in the `meshes_identical` canary test.
...
## Next Steps
- Expand automated test coverage further: the `tests/` suite now has 162 test cases covering palette storage, mesh culling, greedy mesher, LOD, neighbor accessor, face emission, density field, concurrency (shard locking, exclusive serialization, PaletteStorage R/W, cross-chunk writer races, light removal), and the poisoned-solid_cache canary, but edge cases in light propagation BFS paths across chunk boundaries still lack regression tests.
- Optimize the light-checksum scan in partial remeshing: the per-column light checksum pass (~0.08 ms, reading 32×32×1024 cells) is now the dominant cost of an incremental rebuild (0.094 ms total). Options: checksum per 16³ section (8 per chunk) with a coarse invalidation (dirty_subchunks is already maintained), a checksum grid cached across builds keyed on generation counter, or only re-checking columns that overlap the edit/lit region.
