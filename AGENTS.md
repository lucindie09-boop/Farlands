...
- **Data-driven worldgen**: Macro-surface layers load from `data/terrain_config.json`; per-biome surfaces/tree variants load from `data/biomes.json` (climate thresholds load but are dormant — temperature/humidity samplers are flat stubs, so worldgen emits Hills on land and Ocean below sea level)
