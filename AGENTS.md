...
- Worldgen tuning is data-driven: `data/terrain_config.json` (macro-surface layers: base height, warp amps, relief fields, strength range, weirdness thresholds), `data/biomes.json` (per-biome surface materials + tree variants; climate thresholds load but are dormant while climate samplers are flat stubs), `data/vegetation.json` (hills sparse-tree knobs)
