...
- **With the walk cheap, the band frontier is what the sweep's time now is.** Same 40 s flight, splitting `total_ms` by phase: **walk 839 ms, band reads 2,820 ms, rebuilds 37 ms** out of 3,660 ms — and `band_reads` is exactly **397,907 ≈ 124 crossings x 3,209 columns**, because a crossing rebuilds the list and the frontier re-reads every band in it even though a column's band depends only on that column's own surface bounds and not on where the player stands. That, not the walk, is the next thing to fix, and `max_band_ms` (13-35 ms against a 2 ms budget) says the same column cost that used to hitch boot frames is still in the per-column tail
