...
- `src/core/item_registry.hpp/cpp` — `ItemRegistry`, `ItemToolStats`: non-placeable inventory objects in their own id space above blocks, loaded from `data/items.json`, with optional per-item tool stats (`class`/`tier`/`speed`) and an optional in-world use action (`ItemUseAction`, `"use": {"kind", "block"}` — `pour` writes that block into the cell the crosshair is against; resolved by name at load since blocks load first). Entry order is the id, so new entries are appended. The file's optional `"pose"` field is read by `viewmodel.gd` (not the registry) since it selects a viewmodel resting position, not an item mechanic
