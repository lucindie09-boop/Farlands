...
- `pose_clone_debug.gd` — Debug tool (K key): spawns an animated copy of the player model standing on the aimed block, with a bright depth-test-off cube at every mesh's origin (see `shaders/pose_pivot_marker.gdshader`); prints each part's pivot for marker-vs-transform verification
