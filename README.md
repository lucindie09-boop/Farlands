...
- **`data/block_definitions.json`** — the block registry (entry order = save-format ID, so append only), including `hardness`, the `preferred_tool`/`min_tier` pair, and an optional `crush_result` naming the block a **hammer** leaves behind instead of this one (resolved by name, so the target may be declared later in the file; see the hammer row above). JSON has no comments, so notes about an entry go in a `"_comment"` key — the loaders read named fields only and ignore unknown keys
