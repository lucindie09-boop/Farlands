...
| Fluid flow | `src/fluids/` — `fluid_rules.hpp/cpp` | The flow rules with no Godot dependencies: a cell is a source, runoff at its distance from one, or falling at full strength; it recomputes from its neighbours, becomes falling when there is fluid above it, becomes a source when two source neighbours sit over something solid, dries up when nothing feeds it, and pushes fluid down first — otherwise sideways, and only toward the directions nearest a drop (a bounded 4-step search that short-circuits when a neighbour can drop straight off). Scheduling is deliberately not in here. Covered by 13 tests in `tests/test_fluid_rules.cpp`, which runs every scene through both a slow "recompute everything until quiet" driver and the queued one and compares the results cell by cell — but it is **not wired into the game yet**: no block declares a fluid state and nothing calls `tick()`, so a poured bucket is still one still block |
