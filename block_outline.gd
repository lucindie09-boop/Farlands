...
func _rebuild_outline_mesh():
	...
	# Edge geometry is built in C++ (BlockOutlineBuilder) — the XOR per-plane
	# culling plus per-edge quad extrusion was ported to a tested core module.
	var data := BlockOutlineBuilder.build_outline(_current_boxes, outline_thickness)
...
func _update_fill_for_boxes():
	if _current_boxes.is_empty():
		return
	# Union bounds computed in C++ (BlockOutlineBuilder::fill_bounds).
	var data := BlockOutlineBuilder.fill_bounds(_current_boxes)
	var center: Vector3 = data["center"]
	var size: Vector3 = data["size"]
	var pad := 0.001
	fill_mesh.mesh.size = size + Vector3(pad, pad, pad)
	fill_mesh.position = center
