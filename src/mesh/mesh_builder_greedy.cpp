            ...
            for (int32_t x = 0; x < CHUNK_WIDTH; x += stride_xz_) {
                ...
                for (int32_t z = 0; z < CHUNK_DEPTH; z += stride_xz_) {
                    ...
                    if (merge_start != -1 && block_id == current_block
                        ...
                        float block_ao[4];
                        ao.compute_face(accessor, x, y, z, direction, block_ao, stride_xz_);
                    ...
                    current_rotation = rotation;
                    current_light_key = light_key;
                    ao.compute_face(accessor, x, y, z, direction, current_ao, stride_xz_);
                }
                    ...
                    for (int d = 0; d < kDirCount; d++) {
                        ...
                        if (dst.merge_start != -1) {
                            ...
                            if (same_block && within_distance && same_light && same_rotation) {
                                float block_ao[4];
                                this->ao.compute_face(accessor, x, y, z, kDirs[d], block_ao, stride_xz_);
                                if (!dst.ao_valid) {
                                    this->ao.compute_face(accessor, x, dst.merge_start, z, kDirs[d], dst.current_ao, stride_xz_);
                                    dst.ao_valid = true;
                                }
