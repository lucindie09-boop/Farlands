            ...
            for (int32_t x = 0; x < CHUNK_WIDTH; x += stride_xz_) {
                ...
                for (int32_t z = 0; z < CHUNK_DEPTH; z += stride_xz_) {
                    ...
                    if (merge_start != -1 && block_id == current_block
                        ...
                        float block_ao[4];
                        ao.compute_face(accessor, registry, x, y, z, direction, block_ao, stride_xz_, true, true);
                    ...
                    current_rotation = rotation;
                    current_light_key = light_key;
                    ao.compute_face(accessor, registry, x, y, z, direction, current_ao, stride_xz_, true, true);
                }
            ...
            for (int32_t s = 0; s < CHUNK_SECTIONS; ++s) {
                ...
                for (int32_t y = y0; y < y1; y++) {
                    ...
                    if (stride_xz_ > 1) {
                        ...
                        if (block_id == BlockIDs::AIR) greedy_v_stats_local.lod_cells_skipped_air++;
                    }
                    ...
                    for (int d = 0; d < kDirCount; d++) {
                        ...
                        if (dst.merge_start != -1) {
                            ...
                            if (same_block && within_distance && same_light && same_rotation) {
                                float block_ao[4];
                                this->ao.compute_face(accessor, registry, x, y, z, kDirs[d], block_ao, stride_xz_, true, true);
                                if (!dst.ao_valid) {
                                    this->ao.compute_face(accessor, registry, x, dst.merge_start, z, kDirs[d], dst.current_ao, stride_xz_, true, true);
                                    dst.ao_valid = true;
                                }
