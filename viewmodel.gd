...
# Walk bobbing (vanilla walk-bob): _walk_dist accumulates distance walked,
...
# --- SCALED MINECRAFT-STYLE VIEWMODEL LAG (~1.5x Stronger) ---
	...
	# --- Walk bobbing (vanilla walk-bob formula) ---
...
func _update_swing_hooks() -> void:
	...
	var swing_progress := maxf(_swing, _swing_place)  # 1 -> 0 (vanilla swing progress)
...
func _update_arm_animation(current_rot: Vector3, current_pos: Vector3) -> void:
	...
	# Build pose: vanilla arm basis + interpolated HUD rotation values
