...
#
# The held item pose is vanilla 1.8.8's first-person item transform (base offset,
# 45-degree yaw, 0.4 scale doubled for blocks, plus the exact swing/bob math), so
# punch() reproduces the vanilla swing. The arm is an explicit shoulder -> grip limb
# aligned to reach the held item, because vanilla's own arm chain
...
#   renderRightArm() renders the right arm with
...
# under R (Basis(xcol,ycol,zcol) * v == R * v). vanilla and Godot are both
# right-handed, so no mirroring is needed.
#
# The source matrix is exact, but mapping it onto player.glb (a longer arm with
# no material above the shoulder pivot, unlike vanilla's inset-box arm) plus Godot's
...
# diagonal vanilla held-arm in the lower-right of the screen. vanilla's own translations
...
func _ready() -> void:
	...
	# --- Held item (first-person item transform). ---
	...
	scale04.scale = Vector3.ONE * 0.4 # first-person item transform scale
...
func _align_to_grip(root: Node3D) -> Basis:
	...
	# plane, so its flat face points at the viewer like vanilla's first-person arm
...
func _update_swing_hooks() -> void:
	...
	# vanilla bob: (-0.4 sin(sqrt s pi), 0.2 sin(sqrt s pi*2), -0.2 sin(s pi))
	_hand_bob.position = Vector3(-0.4 * f4, 0.2 * sin(sqrt(s) * PI * 2.0) - 0.25 * _equip, -0.2 * sin(s * PI))
	# vanilla swing rotations (first-person item transform): yaw f*-20, roll f1*-20, pitch f1*-80
...
func _update_arm_rotation() -> void:
	if _arm_pivot != null:
		...
		# delta on top, defaulting to no-op so the arm matches vanilla.
